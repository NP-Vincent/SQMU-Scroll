## **SQMU Crowdfund Contract**

### **Features**

- Sells pre-minted governance tokens (token ID `0`) for accepted stablecoins.
- Price per token stored in USD with 18 decimals and adjustable by the owner.
- Accepts USDC, USDT and USDQ on Scroll mainnet.
- Uses token decimals to calculate the total cost when purchasing.
- Non‑reentrant `buy()` function transfers ERC‑20 payment and delivers ERC‑1155 tokens.
- Owner can withdraw accumulated stablecoins with `withdrawPayments()`.
- Fully upgradeable via UUPS and protected with `OwnableUpgradeable`.
- Emits events for purchases, price changes and withdrawals.
