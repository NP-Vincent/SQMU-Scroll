// rent_email_receipt.gs - send rent payment or deposit receipts
function doPost(e) {
  try {
    ...
    if (!(email && txLink && usd && token && chain && prop && stage)) {
      throw new Error('Missing fields');
    }
    let body = 'Rent payment receipt for property ' + prop + '.\n' +
      'Transaction: ' + txLink + '\n' +
      'Amount: ' + usd + ' ' + token + ' on ' + chain + '\n' +
      'Stage: ' + stage;
    MailApp.sendEmail(email, 'Rent Payment Receipt', body);
