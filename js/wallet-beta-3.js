// wallet-beta.js — SQMU unified wallet (MetaMask SDK UMD + Web3Auth UMD v10)
...
// Web3Auth client-side config (no secrets on frontend)
...
function resolveWeb3AuthCtor() {
  // Web3Auth UMD often attaches as Web3auth (lowercase 'a'); detect both
...
let _w3a = null;        // Web3Auth (UMD v10)
let _w3aProvider = null;
let _w3aConnected = false;

async function ensureWeb3Auth() {
  if (_w3a) return _w3a;
  const Web3Auth = resolveWeb3AuthCtor();
  if (!Web3Auth) throw new Error('Web3Auth UMD not found. Check the <script> tag.');
  _w3a = new Web3Auth({
  ...
  // v10 API: init() once, then connect() when user clicks
  await _w3a.init();
  return _w3a;
}
...
export async function connectWallet(statusId, preferred = 'metamask') {
  ...
  if (route === 'web3auth') {
    ...
    _w3aProvider = await w3a.connect();        // shows modal, returns EIP-1193 provider
